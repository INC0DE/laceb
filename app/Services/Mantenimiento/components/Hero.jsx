...
import { Wrench, Settings, PenTool, Shield, ArrowRight } from "lucide-react";
...
const Hero = () => {
  ...
  const floatingIcons = [
    { Icon: Wrench, delay: 0, x: "15%", y: "20%", color: "text-primary-400" },
    { Icon: Settings, delay: 1, x: "80%", y: "30%", color: "text-accent-400" },
    {
      Icon: PenTool,
      delay: 2,
      x: "85%",
    ...
    <section ref={heroRef} className="relative w-full h-screen overflow-hidden">
      ...
      >
        <div className="relative w-full h-full">
          <Image
            ...
            alt="Mantenimiento de pipetas LACEB"
            fill
            priority
        ...
        {/* Overlays con gradiente específico para mantenimiento */}
        <div className="absolute inset-0 bg-gradient-to-r from-accent-900/90 via-accent-900/50 to-transparent" />
      ...
      <div className="absolute inset-0 z-10 flex flex-col justify-center px-4 sm:px-6 lg:px-8 xl:px-12">
        <div className="max-w-5xl">
          ...
          <h1
            ref={titleRef}
            className="text-4xl sm:text-7xl md:text-8xl lg:text-9xl xl:text-[10rem] font-bold text-white mb-4 leading-none"
          >
            MANTENIMIENTO
          </h1>
          ...
          <div ref={subtitleRef} className="space-y-2">
            ...
            <p className="text-lg sm:text-xl text-white/60 max-w-2xl">
              Mantenimiento especializado para garantizar la precisión y vida
              útil de tus pipetas
            </p>
          </div>
          ...
          <div ref={featuresRef} className="mt-12 flex flex-wrap gap-4">
            {[
              { label: "Limpieza profunda", color: "bg-primary-500" },
              { label: "Lubricación", color: "bg-accent-500" },
              { label: "Ajuste de precisión", color: "bg-success-500" },
              { label: "Pruebas de funcionamiento", color: "bg-primary-500" },
            ].map((item, index) => (
              <div
                key={index}
                className={`${item.color} px-4 py-2 rounded-full text-white text-sm font-medium shadow-lg cursor-default hover:scale-105 transition-transform duration-300`}
              >
                {item.label}
              </div>
            ))}
